...
function MainContent() {
	...
	const formik = useFormik({
		...
		validationSchema: yup.object({
			...
			contactTgUsername: yup.string().required('required'),
		...
		onSubmit: async (values) => {
			...
			projectData!.projectName !== values.projectName &&
				Object.assign(formData, { projectName: values.projectName });
			...
			projectData!.currency !== values.currency &&
				Object.assign(formData, { currency: values.currency });

			projectData!.grantAmount !== values.grantAmount &&
				Object.assign(formData, { grantAmount: values.grantAmount });

			// contact
			projectData!.telegramUsername !== values.contactTgUsername &&
				Object.assign(formData, { telegramUsername: values.contactTgUsername });

			projectData!.email !== values.contactEmailAddress &&
				Object.assign(formData, { email: values.contactEmailAddress });

			// projectData!.twitterLinkedinUrl !== values.contactSocialMediaUrl &&
			// 	Object.assign(formData, { twitterLinkedinUrl: values.contactSocialMediaUrl });

			projectData!.walletAddress !== values.contactWalletAddress &&
				Object.assign(formData, { walletAddress: values.contactWalletAddress });
		...
		() => QueryApi.projects.getOne(projectId),
		{
			onSuccess: (data) => {
				formik.setValues({
					...
					contactTgUsername: data.telegramUsername,
					contactEmailAddress: data.email,
					// contactSocialMediaUrl: data.twitterLinkedinUrl,
					contactWalletAddress: data.walletAddress,
				});
			},
		}
	);
										...
										<div className="form-control" key={idx}>
											<label className="flex items-center gap-x-2  h-[48px] p-2.5 bg-light-900 dark:bg-dark-900 rounded-xl cursor-pointer border border-transparent hover:border-ablue-200 hover:dark:border hover:dark:border-ablue-400">
												<input
													...
													value={option.value}
													checked={values.currency === option.value}
												/>
							...
							<div className="field space-y-1">
								...
								<div className="form-control">
									<input
										...
										className={`text-field ${
											touched.grantAmount && errors.grantAmount && 'invalid'
										}`}
...
function Sidebar() {
	...
	useLayoutEffect(() => {
		const sec1 = document.getElementById('project-details')! as HTMLElement;
		const sec2 = document.getElementById('file-upload')! as HTMLElement;
...
export default function EditProjectsAdmin() {
	return (
